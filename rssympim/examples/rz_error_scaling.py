...
def create_init_conds(_ptcl_data, _field_data):
    ...
    _field_data.omega_coords = 10.*_ptcl_data.mc[0]*np.ones((n_r_modes, n_z_modes, 2))
    ...
    _ptcl_data.pz = _ptcl_data.mc * np.arange(0., 1000., 1000. / n_macro_ptcls)
...
plt.loglog(t, (t*t*t)/10**8, label=r'$t^{3}$', alpha=0.5, linestyle='-.')
plt.loglog(t, (t*t)/10**7, label=r'$t^{2}$', alpha=0.5, linestyle='--')
